...
def image_width_calc(height: int, img) -> tuple:
    # Calculate image width to save proportions
    proportion = height / float(img.size[1])
    width = int(float(img.size[0]) * proportion)
    return width, height


def process_image(image, account_tier_size: list, has_original: bool, user_id: int):
    ...
    for i in images:
        ...
        file_name = f"{user_id}_resized_{account_tier_size[images.index(i)]}.{img.format}"
